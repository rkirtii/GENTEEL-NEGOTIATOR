...
def get_args():
    ...
    parser.add_argument("--pretrained_model", type=str, default="../rewards/roberta/roberta-base")
    parser.add_argument("--pretrained_config", type=str, default="../rewards/roberta/roberta-base")
    parser.add_argument("--pretrained_tokenizer", type=str, default="../rewards/roberta/roberta-base")
...
class RewardsModelAgent:
    def __init__(self, args, additional_special_tokens):
        self.args = args
        # prepare model
        self.pretrained_config = RobertaConfig.from_pretrained(args.pretrained_config)
        self.pretrained_config.num_labels = args.n_class
        self.pretrained_tokenizer = RobertaTokenizer.from_pretrained(args.pretrained_tokenizer)
        self.pretrained_tokenizer.add_tokens(additional_special_tokens)
        self.model = RobertaForSequenceClassification.from_pretrained('roberta-base', config='roberta-base').to(args.device)
...
def main():
    args = get_args()
    set_seed(args.seed)
    additional_special_tokens = ["[gradual_concession_making]","[large_initial_concession_making]","[patterned_concession_making]","[logrolling]","[expanding_the_pie]","[active_listening]","[leverage_information]", "[no_strategy]", "[KTS]"]
